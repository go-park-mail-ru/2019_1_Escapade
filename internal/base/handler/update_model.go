...
func (h *Handler) UpdateModel(
	...
	if needAuth {
		...
		if err != nil {
			return h.Fail(http.StatusUnauthorized, err)
		}
	}

	// updated - new version of object - get it from request
	err = h.ModelFromRequest(r, updated)
	if err != nil {
	...
	return h.Success(http.StatusOK, object)
}
