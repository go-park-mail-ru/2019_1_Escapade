...
import (
	...
	"github.com/go-park-mail-ru/2019_1_Escapade/internal/infrastructure/auth"
)
...
func (service *OAuth2) HashPassword(password string) string {
	...
	hasher.Write([]byte(password + service.c.Auth.Salt))
	password = base64.URLEncoding.EncodeToString(
		hasher.Sum(nil),
	)
	hasher.Write([]byte(service.c.Auth.Salt + password))
	password = base64.URLEncoding.EncodeToString(
		hasher.Sum(nil),
	)
	return password
...
func (service *OAuth2) Check(
	...
	if !(service.c.Cookie.LifetimeHours == 0) {
		service.log.Println("look in cookies")
		isReserve := false
		token, err = GetToken(r, service.c.Cookie, isReserve)
		if err != nil {
			isReserve = true
			token, err = GetToken(r, service.c.Cookie, isReserve)
		}
		...
		if err == nil {
			if updated {
				SetToken(rw, isReserve, token, service.c.Cookie)
			}
...
func (service *OAuth2) check(
	...
	if token.TokenType != service.c.Auth.TokenType {
		service.log.Println(
			"TokenType wrong! Get:",
			token.TokenType,
			". Expected:",
			service.c.Auth.TokenType,
		)
		return "",
			oauth2.Token{},
			false,
...
func (service *OAuth2) DeleteToken(
	...
	DeleteToken(rw, false, service.c.Cookie)
	DeleteToken(rw, true, service.c.Cookie)
...
func (service *OAuth2) CreateTokenInCookies(
	...
	SetToken(rw, false, *token, service.c.Cookie)
	...
	SetToken(rw, true, *token, service.c.Cookie)
	return err
}
