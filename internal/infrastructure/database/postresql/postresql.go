...
func New(configuration database.Configuration) *PostgresSQL {
	return &PostgresSQL{
		c: configuration,
	}
}
...
func (db *PostgresSQL) Open() error {
	var err error
	var connStr = os.Getenv("DB_CONN_STRING")
	// TODO вынести куда нибудь поближе к main.go
	// TODO А еще есть идея добавить это полем в конфиг. см. https://github.com/ilyakaznacheev/cleanenv

	db.Db, err = sql.Open(DRIVER, connStr)
	if err != nil {
		return err
	}
