...
func NewPostgresSQL(
	rep infrastructure.DatabaseRepositoryI,
) *PostgresSQL {
	return &PostgresSQL{
		Database: rep.Get(),
	}
...
func (db *PostgresSQL) Open() error {
	var err error
	var connStr = os.Getenv("DB_CONN_STRING") // TODO вынести куда нибудь поближе к main.go
	...
	db.Db.SetConnMaxLifetime(db.MaxLifetime)
	return db.Db.Ping()
}
