...
func UpdateModel(r *http.Request, updated ModelUpdate, needAuth bool,
	...
	if needAuth {
		if userID, err = GetUserIDFromAuthRequest(r); err != nil {
			return NewResult(http.StatusUnauthorized, nil, re.AuthWrapper(err))
		}
	}
	...
	if !updated.Update(object) {
		return NewResult(http.StatusBadRequest, nil, re.NoUpdate())
	}

	// try to set updated object to database
	if err = setToDB(object); err != nil {
		return NewResult(http.StatusInternalServerError, nil, err)
	}
