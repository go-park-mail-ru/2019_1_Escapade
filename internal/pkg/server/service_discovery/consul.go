...
type Consul struct {
	...
	ExtraChecks func(consul *Consul) []*consulapi.AgentServiceCheck
...
func (cs *Consul) Init(input *Input) Interface {
	cs.Input = input
...
func (cs *Consul) register(tags ...string) error {
	...
	if cs.Input.LoadBalancer != nil {
		tags = append(tags,
			cs.Input.LoadBalancer.WeightTags(
				cs.Input.ID,
	...
	if (cs.ExtraChecks != nil) {
		checks = cs.ExtraChecks(cs)
	} 
	...
	err = cs.client().Agent().ServiceRegister(&consulapi.AgentServiceRegistration{
		...
		Tags:    append(cs.Input.tags, tags...),
		Checks:  append(cs.checks, checks...), //https://www.consul.io/docs/agent/checks.html
	})
	if err != nil {
		if err = cs.connect(); err != nil {
			utils.Debug(false, "cant connect to consul", err.Error())
...
func (cs *Consul) HTTPCheck(scheme, path, timeout, interval string) *consulapi.AgentServiceCheck {
...
func (cs *Consul) Health() *consulapi.Health {
