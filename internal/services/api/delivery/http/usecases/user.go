...
func NewUserHandler(authToken config.AuthToken,
	user api.UserUseCaseI,
	record api.RecordUseCaseI,
	rep delivery.RepositoryI) *UserHandler {
	handler := &UserHandler{
		config: authToken,
		user:   user,
		record: record,
...
func (h *UserHandler) CreateUser(rw http.ResponseWriter, r *http.Request) ih.Result {
	...
	err := ih.GetUserWithAllFields(r, h.config.Auth.Salt, &user)
	if err != nil {
	...
	if err != nil {
		return ih.NewResult(http.StatusBadRequest, nil, re.UserExistWrapper(err))
	}

	err = auth.CreateTokenInCookies(rw, user.Name, user.Password,
		h.config.AuthClient.Config, h.config.Cookie)
	if err != nil {
...
func (h *UserHandler) UpdateProfile(rw http.ResponseWriter, r *http.Request) ih.Result {
	...
	if err = ih.GetUser(r, h.config.Auth.Salt, &user); err != nil {
		return ih.NewResult(http.StatusBadRequest, nil, err)
	}
...
func (h *UserHandler) deleteUserInDB(ctx context.Context,
	...
	if err = h.user.DeleteAccount(ctx, user); err != nil {
		return
	}
...
func (h *UserHandler) RandomUsers(c context.Context, limit int, timeout time.Duration) {
	...
	for i := 0; i < limit; i++ {
		...
		for j := 0; j < 4; j++ {
			...
			h.record.Update(ctx, int32(userID), record)
		}
