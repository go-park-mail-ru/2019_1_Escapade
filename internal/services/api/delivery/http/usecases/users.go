...
func NewUsersHandler(user api.UserUseCaseI,
	rep delivery.RepositoryI) *UsersHandler {
...
func (h *UsersHandler) GetOneUser(rw http.ResponseWriter, r *http.Request) ih.Result {
	userID, err := h.rep.GetUserID(r)
	if err != nil {
		return ih.NewResult(http.StatusBadRequest, nil, re.NoUserWrapper(err))
	}
...
func (h *UsersHandler) GetUser(rw http.ResponseWriter, r *http.Request,
	...
	user, err := h.user.FetchOne(r.Context(), userID, difficult)
	if err != nil {
...
func (h *UsersHandler) GetUsersPageAmount(rw http.ResponseWriter, r *http.Request) ih.Result {
	...
	pages.Amount, err = h.user.PagesCount(r.Context(), perPageI)
	if err != nil {
		return ih.NewResult(http.StatusInternalServerError, nil, re.DatabaseWrapper(err))
	}
...
func (h *UsersHandler) GetUsers(w http.ResponseWriter, r *http.Request) ih.Result {
	...
	users, err = h.user.FetchAll(r.Context(), difficultI, pageI, perPageI, sort)
	if err != nil {
		return ih.NewResult(http.StatusNotFound, nil, re.NoUserWrapper(err))
	}
	...
	return ih.NewResult(http.StatusOK, &models.UsersPublicInfo{users}, nil)
}
