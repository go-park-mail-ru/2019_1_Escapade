...
func (lobby *Lobby) sendAndSave(request *Request) {
	message := NewMessage(request)
	API.DB.SaveMessage(message)
	SendToConnections(message, All, lobby.Waiting.Get, lobby.Playing.Get)
}
...
func (lobby *Lobby) Join(newConn *Connection) {
	...
	if err == nil {
		...
		if err == nil {
			newConn.SendInformation(bytes)
			newConn.debug("messages send")
		}
	}
	if err != nil {
		newConn.debug("some error catched:" + err.Error())
	}
